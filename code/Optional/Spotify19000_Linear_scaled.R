...
rm(mod_full_inter, MSE_full_inter, RMSE_full_inter, MAE_full_inter, RSS_full_inter)
...
MAE_bw <- mean(abs(residuals(mod_bw))) #16.03688
MAE_bw
...
#model, and the reduced obtained by backward selection. We get two errors
#in the delta component. The first one is the average mean-squared error that we obtain 
#from doing K-fold CV. The second is the average mean-squared error that you obtain from 
...
###Using 5-fold CV, we obtained the highest RMSE for the null model - 20.38065, 
#and the lowest for the reduced model obtained through backward variable selection
# - 20.18184. Though not significantly lower than the full model - 20.18614, the 
#backward selected model turned out to fit the data best. Because we couldn't get a 
#lower error on the reduced model, we suspect that the reason for this is either
#removing the enrgy variable or that the variable selection is not well,
#so we further use Ridge regression to impose a penalty term and LASSO.